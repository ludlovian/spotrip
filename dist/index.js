...
class Sade {
	...
	parse(arr, opts={}) {
		...
		let argv = lib$1(arr.slice(offset), { alias });
		...
		let vals = lib$1(arr.slice(offset), opts);
...
const writeFile = fs__default['default'].promises.writeFile;
const readFile = fs__default['default'].promises.readFile;
const readdir = fs__default['default'].promises.readdir;
const stat = fs__default['default'].promises.stat;
...
async function recordTrack (uri, flacFile) {
  ...
  await dist$2(() => captureTrackPCM(uri, pcmFile, { onProgress }), {
...
async function extractMp3 (path$1) {
  ...
  for (const track of tracks) {
    ...
    await convertToFlac$1(mp3File, flacFile);
    const tags = await readTrackTags$1(mp3File);
    if (trackNumber === 1) {
...
async function convertToFlac$1 (mp3File, flacFile) {
